...
| `assessment.eval_injection_suspicion_threshold` | float [0,1] | `0.5` **(proposed)** | D-22 | Generated eval questions whose injection-suspicion score exceeds this threshold are forced to `review_status=unreviewed` rather than being auto-accepted. Lower values are more conservative (more questions flagged for review). | D-22 |
| `assessment.sweep_min_corpus_docs` | int ≥ 1 | `50` **(proposed)** | §9.3, M-050, D-07 | Below this document count the platform declines to run a configuration sweep and applies the reference configuration instead. Set to 50 per M-050 (small-corpus decline threshold). Type is `int` (not `int \| None`) — the field always has a meaningful value after Phase 5. | spec §9.3, M-050, D-07 |
| `assessment.sweep_candidate_budget` | int ≥ 1 | `40` **(proposed)** | D-07 | Maximum number of candidate configurations evaluated per evolutionary-search sweep iteration. Keeps sweep cost bounded. | D-07 |
| `assessment.sweep_sample_factor` | int ≥ 1 | `2` **(proposed)** | D-07 | Multiplier applied to `sweep_candidate_budget` when sampling the initial candidate pool (initial pool = `sweep_candidate_budget × sweep_sample_factor`). Allows a larger exploration space before culling to the budget. | D-07 |
...
| OQ-C-4 | **PARTIALLY CLOSED (Phase 5).** `assessment.sweep_min_corpus_docs` set to `50` per M-050/D-07; `assessment.sweep_candidate_budget` set to `40` and `assessment.sweep_sample_factor` set to `2` per D-07. `cache.query_embedding.max_entries` remains open pending Phase 5 sizing. `retrieval.rate_limit.queries_per_second_per_tenant` proposed as `10` in Phase 4 (ADR-0008). | spec §9.3, §11.3 |

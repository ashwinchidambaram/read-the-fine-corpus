...
| `assessment.boilerplate_corpus_proportion` | float [0,1] | `0.30` **(proposed)** | §6.2 | A text block appearing in more than this proportion of corpus documents is classified as `boilerplate`. | segment-taxonomy.md OQ-8 |
| `assessment.boilerplate_small_corpus_proportion` | float [0,1] | `0.50` **(proposed)** | §6.2 | Boilerplate threshold used when the corpus has fewer documents than `assessment.boilerplate_small_corpus_doc_count`. Raises the bar to avoid false positives in small corpora. | segment-taxonomy.md OQ-8 |
| `assessment.boilerplate_small_corpus_doc_count` | int | `10` **(proposed)** | §6.2 | Corpus size (document count) below which the small-corpus boilerplate proportion applies. | segment-taxonomy.md OQ-8 |

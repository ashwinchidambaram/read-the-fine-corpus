...
| `text` | `str` | yes | **The chunk's source text.** Byte-identical to the span of the **Tier-1-normalized** canonical source unless a `tier=3, changed_text=true` transformation is recorded in `provenance.transformations` (§7.2, §12; see "Canonical source text" invariant). When rewritten, this holds the **rewritten** form. The RAW original is always retained and addressable via `provenance.source_location`. This is what the caller receives. |
| `original_text` | `str \| null` | no | **The canonical Tier-1 text this chunk was rewritten *from*** (§7.2 C-R7, D-14). Populated **only** when a `tier=3, changed_text=true` transformation rewrote the chunk (`text` then holds the rewrite); `null` for every non-Tier-3 chunk. Surfaced at citation time (retrieval-response) so the original is **always visible** alongside the rewrite. Added in schema 1.1.0 (nullable/defaulted; 1.0.0 consumers still validate). |

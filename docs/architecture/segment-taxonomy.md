...
**Status:** Proposed — awaiting product owner review (Open Decision #3)
...
| `excluded` | The segment is unservable or has been explicitly excluded by user configuration. Scanned regions below the minimum OCR confidence threshold (user-configurable), near-duplicate superseded segments, content the user has manually excluded. | Excluded from retrieval by default. Retained in the index for provenance and for re-inclusion if the exclusion decision changes. Never permanently discarded at ingestion. | Filtered out by default. Must be explicitly included by caller filter. |

### 3.2 The indexing invariant

Everything is indexed. Tier controls default retrieval behaviour only. This is the correct answer to the bloated-manual problem (§6.3): tier assignments can be changed in config; discarded content cannot be recovered without re-running ingestion.
...
| Document with heavy near-duplicate siblings | `prose`, `heading`, and whatever the document normally contains | `primary`, `supporting` for the primary version; `excluded` for superseded near-duplicates | Near-duplicate clustering (§6.1) marks superseded versions. Their segments are decomposed and typed, but the version flag sets them to `excluded` by default. The newest member is the primary version at full tier. |
...
| **OQ-11** | §6.1 says superseded near-duplicate documents are "retained as superseded and **excluded from the index** by default", while §6.3 mandates everything-is-indexed with tier controlling retrieval. This page maps superseded siblings to tier `excluded` (indexed, filtered by default) — which satisfies §6.3's philosophy but reads against §6.1's letter. | §5.2 near-duplicate fixture row; §6.1 vs §6.3 | Proposed default: superseded versions ARE embedded and indexed at tier `excluded`, so a version-family question ("what changed since rev 2?") remains answerable by explicit filter and un-superseding is a config change (metadata-only tier re-bake, index-lifecycle §7.5), not a reingest. If corpus-scale cost makes embedding superseded copies too expensive, revisit with evidence. | **Ruling proposed, owner sign-off pending.** Orchestrator-raised (review finding); a real spec tension, not a drafting gap. |
...
- **Salience tiering, not pruning (§6.3).** The `excluded` tier is not a deletion. Every segment is indexed. Tier controls default retrieval behaviour.

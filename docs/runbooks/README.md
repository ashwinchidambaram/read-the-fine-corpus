...
|---|---|---|---|
| [`connectors.md`](connectors.md) | Operator configuring a source connector (Google Drive, SharePoint, Atlassian/Confluence, Atlassian/Jira) with real credentials, or verifying after a credential rotation | Phase 6 | How an operator supplies real OAuth credentials via **environment variables only** (never config files — §14.2), completes the OAuth handshake, stores the token by reference in the control-plane `TokenStore`, and runs a **live** smoke test (CI cannot, since it uses recorded fixtures). Covers per-connector minimum scopes, interpreting `permission_fidelity` and the §14.3 fail-closed gate (including why every Jira issue is `unavailable`), incremental/delta sync, and failure modes. |

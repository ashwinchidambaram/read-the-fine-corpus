...
|---|---|---|---|
| `ingest.md` | Operator starting a new ingestion job | Phase 1 | How to create a KB, connect a source, run `corpus preflight --config corpus.yaml`, review and approve the Plan output, run `corpus pipeline run --source <dir> --artifacts <dir> --run-id <id> --workspace <id> --kb <id>`, monitor progress, interpret the findings report, and handle pause/resume. Includes how to identify whether a pause was caused by provider unavailability or a budget cap. |
